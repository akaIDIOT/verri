...
A set of opinionated yet tasty and simple calendar-based PEP-440 compliant versioning schemes to set-and-forget for
...
Both of these flavours will mark the version as "dirty" if tracked files contain uncommitted changes when the version is
...
`verri` was created with `pdm-backend` in mind, so deliberately exposes its flavours as callable functions. See
